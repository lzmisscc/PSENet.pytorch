...
def draw_bbox(img_path, result, color=(255, 0, 0), thickness=2):
    ...
    for point in result:
        ...
        tmp_path = 'picture_patch'
        os.makedirs(tmp_path, exist_ok=True)
        save_name = tmp_path + '/' + uuid.uuid4().hex + '.jpg'
        left = min([i[0] for i in point])
        top = min([i[1] for i in point])
        right = max([i[0] for i in point])
        button = max([i[1] for i in point])
        cv2.imwrite(save_name, img_path[top:button, left:right, :])
