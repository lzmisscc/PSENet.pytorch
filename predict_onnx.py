...
class PSE_model:
    def __init__(self, model_path, gpu_id=None):
        ...
        '''
        self.gpu_id = gpu_id
        net = PSENet(backbone='resnet18', pretrained=False)
    ...
    def predict(self, img, scale_w, scale_h, ):
        h, w, c = img.shape
        new_h = (h // 2 + 1) * 2
        new_w = (w // 2 + 1) * 2
        ...
        with torch.no_grad():
            ...
            self.net.eval()
            torch.onnx.export(self.net, tensor, 'lsz_key_word.onnx', input_names=['input'], output_names=['output'],
                              opset_version=11, verbose=True,
                              dynamic_axes={"input": {0: "b", 1: "c", 2: "h", 3: "w"},
                                            "output": {0: "b", 1: "c", 2: "h", 3: "w"}})
            # torch.cuda.synchronize(self.device)
            preds, boxes_list, _ = pse_decode(preds, scale=1, threshold=0.7311)
