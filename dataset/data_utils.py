...
def image_label(im_fn: str, text_polys: np.ndarray, text_tags: list, n: int, m: float, input_size: int,
                defrees: int = 10,
                scales: np.ndarray = np.array([i / 10 for i in range(4, 30)])) -> tuple:
    '''
...
class MyDataset(data.Dataset):
    ...
    def load_data(self, data_dir: str) -> list:
        data_list = []
        for x in glob.glob(data_dir + '/*.jpg', recursive=True):
            d = pathlib.Path(x)
            label_path = os.path.join(d.parent, str(d.stem + '.txt'))
...
if __name__ == '__main__':
    ...
    train_data = MyDataset(config.testroot, data_shape=config.data_shape, n=config.n, m=config.m,
